...
namespace lab5
{
    static class Matrix
    {
        ...
        static public double[,] Add(double[,] matrix1, double[,] matrix2)
        {
            double[,] matrix;
            ...
            if (isValidForAddition(rows_matrix1, columns_matrix1, rows_matrix2, columns_matrix2))
            {
        ...
        static private bool isValidForAddition(int rows1, int columns1, int rows2, int columns2)
        {
        ...
        static public double[,] Multiply(double[,] matrix1, double[,] matrix2)
        {
            ...
            if (isValidForMultiply(columns_matrix1, rows_matrix2))
            {
        ...
        static private bool isValidForMultiply(int columns1, int rows2)
        {
            if (rows2 == columns1)
                return true;
            else
        ...
        static public double[,] getInverse(double[,] matrix)
        {
            ...
            if (isSquare(rows, columns))
            {
                ...
                if (determinant != 0)
                {
                    ...
                    matrix = FindTransparent(matrix);            

                    for (int i = 0; i < rows; i++)
                        for (int j = 0; j < rows; j++)
                        {
                            algebraic_complement_matrix[i, j] = Math.Pow(-1, (i + j)) * CalculateDeterminant(GetAddition(matrix, minor, i, j, rows), rows - 1);
                        }
        ...
        static public double CalculateDeterminant(double[,] matrix, int rows)
        {
            ...
            else
            {
                for (int i = 0; i < rows; i++)
                {
                    det += k * matrix[i, 0] * CalculateDeterminant(GetAddition(matrix, new_matrix, i, 0, rows), rows - 1);
                    k = -k;
                }
                return det;
            }
        ...
        static public double[,] GetAddition(double[,] matrix, double[,] p, int i, int j, int m)
        {
            int ki, kj, di, dj;
            di = 0;
        ...
        static public double[,] FindTransparent(double[,] matrix)
        {
            int rows = matrix.GetUpperBound(0) + 1;
            int columns = matrix.Length / rows;
            double temp;
